...
use headshell_core::plugin::{PluginProvider, load};
...
async fn a_plugin_that_speaks_another_protocol_version_is_refused_not_fatal() {
    ...
    assert!(
        matches!(
            err.kind(),
            headshell_core::ErrorKind::PluginIncompatible {
                plugin_api: 99,
                host_api: 1,
                ..
            }
...
async fn a_plugin_that_dies_mid_call_is_isolated_and_restarted() {
    ...
    let err = provider.search("ezhel", 10).await.unwrap_err();
    assert!(
        matches!(err.kind(), headshell_core::ErrorKind::PluginCrashed { .. }),
        "{}",
        err.chain_text()
    );
...
async fn a_hanging_plugin_times_out_instead_of_freezing_the_core() {
    ...
    match err.kind() {
        headshell_core::ErrorKind::PluginTimeout { method, .. } => assert_eq!(method, "handshake"),
