...
//! `headshell secret set identity:acoustid api_key <anahtar>` — ya da doğrudan
//! `HEADSHELL_ACOUSTID_KEY` ortam değişkeni.
    ...
    if !format!("{lookup:?}").contains("api_key_set: true") {
        eprintln!(
            "{test}: AcoustID anahtarı yok — atlanıyor (bu bir başarısızlık değil; \
             `HEADSHELL_ACOUSTID_KEY` ya da `headshell secret set identity:acoustid api_key`)"
        );
        return None;
    }
...
async fn the_committed_fixture_still_matches_what_the_service_sends() {
    ...
    let http = headshell_core::net::default_http_client().expect("http-client açık");
    let url = format!(
