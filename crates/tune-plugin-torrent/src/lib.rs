...
//! `librqbit` `tune-core`'un bağımlılık ağacına 179 crate ekliyordu (77 → 256)
...
const TORZNAB_MISSING: &str = concat!(
    ...
    "`tune secret set plugin:torrent torznab_url` ",
...
pub async fn resolve_source(app: &App, params: serde_json::Value) -> Result<serde_json::Value> {
    ...
    let index = match wanted_index {
        Some(index) => {
            if !files.iter().any(|file| file.index == index) {
                ...
                return to_value(&tune_core::plugin::protocol::ResolveSourceResult {
                    source: None,
                });
            }
            index
            ...
            many => {
                ...
                return err(format!(
                    "bu yayımda {} ses dosyası var, hangisi olduğunu söylemediniz. \
                     `tune provider search torrent {infohash}` dosyaları listeler; \
    ...
    to_value(&tune_core::plugin::protocol::ResolveSourceResult {
...
mod tests {
    ...
    fn the_missing_torznab_message_tells_the_user_exactly_what_to_type() {
        assert!(TORZNAB_MISSING.contains("tune secret set plugin:torrent torznab_url"));
