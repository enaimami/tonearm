...
use tune_core::plugin::protocol::method;
use tune_plugin_torrent::rpc::{CODE_METHOD_NOT_FOUND, CODE_PLUGIN_ERROR, Incoming};
use tune_plugin_torrent::{App, dispatch, rpc};

#[tokio::main(flavor = "multi_thread")]
async fn main() {
    // stdout protokole ait; her günlük satırı stderr'e. Çekirdek stderr'i
    // `tracing`'e aktarıyor, yani librqbit'in teşhisi `tune diag`'a ulaşır.
    let filter = tracing_subscriber::EnvFilter::try_from_env("TUNE_TORRENT_LOG")
