...
/// şey de bu: kullanıcı hiçbir anahtar vermeden `tune play` diyebiliyor.
#[tokio::test]
...
async fn a_soundcloud_track_actually_plays() {
    ...
    let mut registry = tune_core::provider::ProviderRegistry::new();
    ...
    let hits = registry
        ...
        .await
        .unwrap();
    let item = tune_core::playback::QueueItem {
        id: hits[0].id.clone(),
        track: hits[0].track.clone(),
    };

    let mut player = tune_core::playback::Player::new(registry);
