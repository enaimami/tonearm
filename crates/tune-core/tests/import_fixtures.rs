...
use tune_core::identity::ResolveMethod;
use tune_core::model::{ExportKind, PlayRule};
...
fn temp_dir(label: &str) -> PathBuf {
    let unique = std::time::SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    let dir = std::env::temp_dir().join(format!("tune-test-{label}-{unique}"));
...
async fn unsupported_archive_fails_at_the_detect_stage() {
    ...
    // Hata da tanı raporuna yazılmış olmalı — `tune diag` bunu göstermeli.
    let diag = sess
        .last_diag()
        .unwrap()
        .expect("son çalıştırma kaydedilmeli");
    assert!(!diag.succeeded());
    assert_eq!(diag.failed_at, Some(tune_core::diag::Stage::ImportDetect));
...
async fn resolve_reports_the_chain_step_it_used() {
    ...
    let err = sess
        ...
        .await
        .unwrap_err();
    assert_eq!(err.stage(), tune_core::diag::Stage::IdentityResolve);
