...
use tune_core::plugin::consent::ConsentStore;
use tune_core::plugin::manifest::{Permissions, PluginManifest};
use tune_core::plugin::{PluginProvider, load};
...
async fn a_plugin_that_speaks_another_protocol_version_is_refused_not_fatal() {
    ...
    assert!(
        matches!(
            err.kind(),
            tune_core::ErrorKind::PluginIncompatible {
                plugin_api: 99,
                host_api: 1,
                ..
            }
...
async fn a_plugin_that_dies_mid_call_is_isolated_and_restarted() {
    ...
    let err = provider.search("ezhel", 10).await.unwrap_err();
    assert!(
        matches!(err.kind(), tune_core::ErrorKind::PluginCrashed { .. }),
        "{}",
        err.chain_text()
    );
...
async fn an_unapproved_plugin_is_not_loaded_but_is_visible() {
    ...
    let registry = tune_core::provider::registry_with_http(&config, None).unwrap();
...
async fn a_hanging_plugin_times_out_instead_of_freezing_the_core() {
    ...
    match err.kind() {
        tune_core::ErrorKind::PluginTimeout { method, .. } => assert_eq!(method, "handshake"),
