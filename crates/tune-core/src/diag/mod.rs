...
impl EnvInfo {
    ...
    pub fn collect(data_dir: Option<PathBuf>) -> Self {
        Self {
            tune_version: env!("CARGO_PKG_VERSION").to_owned(),
...
/// Son çalıştırmanın tanı raporu. `tune diag` bunu basar.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
impl DiagReport {
    ...
    pub fn render(&self) -> String {
        ...
        let _ = writeln!(
            out,
            "sürüm     : tune {} ({} {})",
            self.env.tune_version, self.env.os, self.env.arch
        );
