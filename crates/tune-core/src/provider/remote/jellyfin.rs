...
/// bu adla görünürüz.
const CLIENT_NAME: &str = "tune";
const DEVICE_NAME: &str = "tune-core";
        ...
        match &self.server.auth {
            ...
            StoredAuth::SubsonicToken { .. } => Err(Error::new(
                ...
                ErrorKind::InvalidInput {
                    detail: format!(
                        "{} kaydı Jellyfin ama Subsonic token'ı taşıyor; `tune provider add` ile yeniden kaydedin",
                        self.server.id
                    ),
                },
            )),
...
mod tests {
    ...
    async fn credentials_travel_in_the_header_not_the_url() {
        ...
        assert!(auth.value.contains("Client=\"tune\""), "{auth:?}");
    }
