...
//! `tune play`'in ürettiği scrobble'lardan gelir.
...
pub struct RemoteServer {
    /// Sağlayıcı kimliği: `tune provider test <bu>`.
    pub id: ProviderId,
    pub kind: ServerKind,
...
mod tests {
    ...
    fn servers_survive_a_file_round_trip_and_stay_private() {
        let dir = std::env::temp_dir().join(format!("tune-servers-{}", std::process::id()));
    ...
    fn a_missing_file_is_an_empty_list_but_a_broken_one_is_an_error() {
        let dir = std::env::temp_dir().join(format!("tune-servers-bad-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
