...
//! gelmeli.
//!
//! Onay verilmemiş bir eklenti **yüklenmez ama görünür**: `tune plugin list`
...
impl ConsentStatus {
    ...
    pub fn describe(&self) -> String {
        match self {
            ...
            Self::NotAsked => "onay bekliyor — `tune plugin approve <ad>`".to_owned(),
            Self::NeedsApproval { extra } => format!(
                "yeni izin istiyor ({}) — `tune plugin approve <ad>`",
                extra.describe()
            ),
            Self::Disabled => "kapalı — `tune plugin enable <ad>`".to_owned(),
        }
    }
}
