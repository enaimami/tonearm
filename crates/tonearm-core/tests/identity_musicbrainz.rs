...
use tonearm_core::identity::musicbrainz::MusicBrainzLookup;
use tonearm_core::identity::{MetadataLookup, ResolveMethod, Resolver};
use tonearm_core::ids::{CanonicalKind, Isrc};
...
fn skip_if_unreachable<T>(test: &str, result: tonearm_core::Result<T>) -> Option<T> {
    use tonearm_core::error::ErrorKind;
    match result {
        Ok(value) => Some(value),
        Err(err) => {
