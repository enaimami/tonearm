...
use tonearm_core::identity::ResolveMethod;
use tonearm_core::model::{ExportKind, PlayRule};
use tonearm_core::session::{self, Session};
...
fn temp_dir(label: &str) -> PathBuf {
    let unique = std::time::SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    let dir = std::env::temp_dir().join(format!("tonearm-test-{label}-{unique}"));
...
async fn unsupported_archive_fails_at_the_detect_stage() {
    ...
    let err = sess
        ...
        .await
        .unwrap_err();
    assert_eq!(err.stage(), tonearm_core::diag::Stage::ImportDetect);
    assert!(
    ...
    // Hata da tanı raporuna yazılmış olmalı — `tonearm diag` bunu göstermeli.
    let diag = sess
        .last_diag()
        .unwrap()
    ...
    assert_eq!(
        diag.failed_at,
        Some(tonearm_core::diag::Stage::ImportDetect)
    );
...
async fn resolve_reports_the_chain_step_it_used() {
    ...
    let err = sess
        ...
        .await
        .unwrap_err();
    assert_eq!(err.stage(), tonearm_core::diag::Stage::IdentityResolve);
