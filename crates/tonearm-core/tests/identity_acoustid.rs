...
//! `tonearm secret set identity:acoustid api_key <anahtar>` — ya da doğrudan
//! `TONEARM_ACOUSTID_KEY` ortam değişkeni.
...
use tonearm_core::identity::FingerprintLookup;
use tonearm_core::identity::acoustid::AcoustIdLookup;
use tonearm_core::identity::fingerprint::fingerprint_file;
...
fn lookup_or_skip(test: &str) -> Option<AcoustIdLookup> {
    let key = std::env::var("TONEARM_ACOUSTID_KEY").unwrap_or_default();
    let http = tonearm_core::net::default_http_client().ok()?;
    let lookup = AcoustIdLookup::new(http)
        .with_api_key(key)
        .with_user_agent("tonearm-tests/0.0.1 ( https://github.com/enaimami/tonearm )");
    ...
    if !format!("{lookup:?}").contains("api_key_set: true") {
        eprintln!(
            "{test}: AcoustID anahtarı yok — atlanıyor (bu bir başarısızlık değil; \
             `TONEARM_ACOUSTID_KEY` ya da `tonearm secret set identity:acoustid api_key`)"
        );
        return None;
    }
...
async fn the_committed_fixture_still_matches_what_the_service_sends() {
    ...
    let http = tonearm_core::net::default_http_client().expect("http-client açık");
    let url = format!(
        "https://api.acoustid.org/v2/lookup?client={key}&trackid={TRACK_ID}\
         &meta=recordings&format=json"
    );
    let response = match http.send(&tonearm_core::net::HttpRequest::get(&url)).await {
...
async fn acoustid_rejects_a_corrupted_fingerprint_so_the_positive_test_means_something() {
    ...
    let request = tonearm_core::net::HttpRequest::post_form(
...
async fn a_rejected_key_is_an_error_not_an_empty_result() {
    ...
    let Ok(http) = tonearm_core::net::default_http_client() else {
        return;
    };
