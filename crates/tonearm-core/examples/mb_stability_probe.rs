...
//!
//! ```bash
//! cargo run -p tonearm-core --features http-client --example mb_stability_probe -- "Sanatçı" "Başlık"
//! ```

use tonearm_core::identity::{MetadataLookup as _, Resolver};
...
async fn main() {
    ...
    let lookup = std::sync::Arc::new(
        tonearm_core::identity::musicbrainz::MusicBrainzLookup::new(http)
        ...
        match lookup.search_recordings(&artist, &title).await {
            Ok(found) => {
                ...
                let mut scored: Vec<(f64, String, Option<u64>)> = found
                    .iter()
                    .map(|c| {
                        let score = tonearm_core::identity::fuzzy::similarity(
                            &artist,
                            &title,
                            duration_ms,
                            &c.artist,
