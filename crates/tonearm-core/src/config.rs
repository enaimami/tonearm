...
pub const DATA_DIR_ENV: &str = "TONEARM_DATA_DIR";

/// Müzik dizinlerini elle vermek için ortam değişkeni (`:` ile ayrılmış).
pub const MUSIC_DIRS_ENV: &str = "TONEARM_MUSIC_DIRS";
...
impl Config {
    ...
    ///
    /// Sıra: `TONEARM_DATA_DIR` → `XDG_DATA_HOME/tonearm` → `HOME/.local/share/tonearm`.
    ///
    /// # Errors
    ...
    pub fn discover() -> Result<Self> {
        ...
        if let Some(dir) = non_empty_env("XDG_DATA_HOME") {
            return Ok(Self::with_data_dir(PathBuf::from(dir).join("tonearm")));
        }
        if let Some(home) = non_empty_env("HOME") {
            return Ok(Self::with_data_dir(
                PathBuf::from(home).join(".local/share/tonearm"),
            ));
        }
        Err(Error::new(
            Stage::ConfigLoad,
    ...
    ///
    /// Sıra: `TONEARM_MUSIC_DIRS` (`:` ile ayrılmış) → `XDG_MUSIC_DIR` →
...
mod tests {
    ...
    fn paths_hang_off_the_data_dir() {
        let config = Config::with_data_dir("/veri/tonearm");
        assert_eq!(
            config.database_path(),
            PathBuf::from("/veri/tonearm/library.db")
        );
        assert_eq!(
            config.last_run_path(),
            PathBuf::from("/veri/tonearm/last-run.json")
        );
        assert_eq!(
            config.servers_path(),
            PathBuf::from("/veri/tonearm/servers.json")
        );
    }
}
