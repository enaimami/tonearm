...
impl EnvInfo {
    ...
    pub fn collect(data_dir: Option<PathBuf>) -> Self {
        Self {
            tonearm_version: env!("CARGO_PKG_VERSION").to_owned(),
...
/// Son çalıştırmanın tanı raporu. `tonearm diag` bunu basar.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
impl DiagReport {
    ...
    pub fn render(&self) -> String {
        ...
        let _ = writeln!(out, "── tonearm diag ───────────────────────────────");
        ...
        let _ = writeln!(
            out,
            "sürüm     : tonearm {} ({} {})",
            self.env.tonearm_version, self.env.os, self.env.arch
        );
...
mod tests {
    ...
    fn sample() -> DiagReport {
        DiagReport {
            ...
            env: EnvInfo {
                tonearm_version: "0.0.0".to_owned(),
                os: "linux".to_owned(),
