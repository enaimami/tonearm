...
pub enum AudioSource {
    ...
    ///
    /// İstemci bunu **kendisi** çeker; `tonearm` sunucusu araya girmez.
    HttpStream {
        url: String,
...
/// Bir sağlayıcının sağlık durumu. `tonearm provider test <ad>` bunu basar.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Kayıtlı sağlayıcılar. `tonearm provider list` bunu okur.
///
...
pub fn registry_with_http(
    ...
    if summary.discovered > 0 {
        // Yüklenmeyenler sessiz kalmasın: sebepleri `tonearm plugin list`'te.
        tracing::debug!(
