...
impl LocalProvider {
    ...
    /// etiketleri değiştirdiyse `tonearm provider scan` demeli; bunu bilmek,
        ...
        Box::pin(async move {
            ...
            Ok(ProviderHealth {
                ...
                detail: if missing.is_empty() {
                    (count == 0).then(|| {
                        "indeks boş — `tonearm provider scan` çalıştırılmamış olabilir".to_owned()
                    })
                } else {
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        let dir = std::env::temp_dir().join(format!("tonearm-local-{label}-{unique}"));
