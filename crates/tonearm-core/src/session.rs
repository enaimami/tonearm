...
/// `tonearm plugin install` çıktısı (D-055).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
pub struct ScanReport {
    /// Taranan kök dizinler. Boşsa kullanıcı `TONEARM_MUSIC_DIRS` vermemiş.
...
impl Session {
        ...
        let result = async {
            ...
            if servers.iter().any(|existing| existing.id == spec.id) {
                ...
                return Err(Error::new(
                    ...
                    ErrorKind::InvalidInput {
                        detail: format!(
                            "{} adında bir sunucu zaten kayıtlı; önce `tonearm provider remove {}`",
                            spec.id, spec.id
                        ),
                    },
                ));
    ...
    pub async fn queue_from_search(
        ...
        if items.is_empty() {
            return Err(Error::new(
                ...
                ErrorKind::NotFound {
                    what: format!(
                        "{query:?} ile eşleşen parça (indeks boşsa: `tonearm provider scan`)"
                    ),
                },
            ));
        }
    ...
    ///
    /// Rapor hem başarıda hem başarısızlıkta yazılır — `tonearm diag` en çok
    ...
    if found {
        ...
        Err(Error::new(
            ...
            ErrorKind::PluginNotApproved {
                ...
                detail: "onay defterinde kaydı yok — önce `tonearm plugin approve`".to_owned(),
            },
        ))
    }
}
...
///
/// Ağa çıkmak **açık bir tercih**: `tonearm` ağ olmadan da eksiksiz çalışan bir
