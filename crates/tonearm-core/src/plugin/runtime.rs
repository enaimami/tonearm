...
//!
//! - [`RequirementState::Missing`] — hiç kurulmadı. `tonearm plugin install`.
...
pub struct PythonInfo {
    ...
    pub version: String,
    /// Nereden bulundu: `TONEARM_PYTHON`, `python3`, `python`. Kullanıcı
...
///
/// Ayrı duruyor çünkü **geri düşülmez**: `TONEARM_PYTHON` verilmişse ve o
    ...
    Err(runtime_err(
        ...
        format!(
            "çalıştırılabilir bir Python {}.{}+ bulunamadı. Denenenler — {}. \
             `tonearm` eklentileri Python ile çalışır; kurulu bir yorumlayıcının \
             yolunu `TONEARM_PYTHON` ortam değişkeniyle verebilirsiniz.",
            MIN_PYTHON.0,
            MIN_PYTHON.1,
            attempts.join(" | ")
        ),
...
fn resolve_override(path: &Path) -> Result<PythonInfo> {
    let version = probe_python(path).map_err(|detail| {
        runtime_err(
            "Python arama",
            format!(
                "`TONEARM_PYTHON` {} çalıştırılamadı: {detail}. Başka bir yorumlayıcıya \
    ...
    match parse_version(&version) {
        Some((major, minor)) if (major, minor) >= MIN_PYTHON => Ok(PythonInfo {
            ...
            version,
            source: "TONEARM_PYTHON".to_owned(),
        }),
        Some((major, minor)) => Err(runtime_err(
            "Python arama",
            format!(
                "`TONEARM_PYTHON` {} sürüm {major}.{minor} gösteriyor; en az {}.{} gerekiyor. \
        ...
        None => Err(runtime_err(
            "Python arama",
            format!(
                "`TONEARM_PYTHON` ({}) sürümünü anlaşılır biçimde bildirmedi: {version}",
                path.display()
            ),
        )),
    }
...
/// Bir eserin adı ve durumu — `tonearm plugin list` bunu basar.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
///
/// Motorun kurulum yolu bir kabuk komutundan (`tonearm plugin install`) çağrılıyor
...
mod tests {
    ...
    fn a_python_that_does_not_exist_names_every_candidate_it_tried() {
        ...
        let probe = probe_python(Path::new("bu-yorumlayici-yok-tonearm-testi"));
        assert!(probe.is_err(), "olmayan yorumlayıcı çalıştı");
    }

    /// Açıkça gösterilen bir yorumlayıcı çalışmıyorsa motor **kaymaz.**
    ///
    /// Bu davranış bir ölçümden doğdu: ilk yazımda `TONEARM_PYTHON` yalnızca
    ...
    fn an_explicit_python_that_does_not_work_is_not_silently_replaced() {
        ...
        assert!(text.contains("TONEARM_PYTHON"), "{text}");
        assert!(
        ...
        assert!(
            python_candidates()
                .iter()
                .all(|(source, _)| source != "TONEARM_PYTHON"),
