...
//!    ([`consent`]) sorulur, protokol sürümü karşılaştırılır. `tonearm plugin
//!    list` bu kadarını kullanır ve hiçbir eklentiyi başlatmaz.
//! 2. **Başlatma** — **ilk çağrıda**, tembel. `tonearm stats` çalışırken
...
//! bu, `tonearm plugin list` çıktısında da böyle yazar.
    ...
    pub fn status_text(&self) -> String {
        ...
        if !missing.is_empty() {
            let detail = missing
                ...
                .join("; ");
            return format!(
                "motorun kurması gereken eser eksik ({detail}) — `tonearm plugin install {}`",
                self.name
            );
        }
        match &self.consent {
...
/// `TONEARM_PYTHON` ile yaptığı seçim de böylece bütün eklentiler için bir kez
/// geçerli olur.
///
...
/// motorun ayrıntılı tanısı `tonearm plugin install` ile alınır.
