...
//! gelmeli.
//!
//! Onay verilmemiş bir eklenti **yüklenmez ama görünür**: `tonearm plugin list`
...
impl ConsentStatus {
    ...
    pub fn describe(&self) -> String {
        match self {
            ...
            Self::NotAsked => "onay bekliyor — `tonearm plugin approve <ad>`".to_owned(),
            Self::NeedsApproval { extra } => format!(
                "yeni izin istiyor ({}) — `tonearm plugin approve <ad>`",
                extra.describe()
            ),
            Self::Disabled => "kapalı — `tonearm plugin enable <ad>`".to_owned(),
        }
    }
}
