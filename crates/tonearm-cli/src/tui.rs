...
//! karıştırma, scrobble — hepsi `tonearm-core` içinde.
//!
...
use tonearm_core::playback::{LiveSession, PlayState, PlaybackAnchor, Player, RepeatMode};
...
fn draw_now_playing(frame: &mut ratatui::Frame<'_>, area: Rect, view: &View<'_>) {
    ...
    frame.render_widget(
        Paragraph::new(line).block(Block::default().borders(Borders::ALL).title(" tonearm ")),
        area,
    );
}
...
fn draw_footer(frame: &mut ratatui::Frame<'_>, area: Rect, view: &View<'_>) {
    if let Some(error) = view.error {
        ...
        // tamamı `tonearm diag` ile okunur.
        let first = error.lines().next().unwrap_or(error);
        frame.render_widget(
            Paragraph::new(Line::from(Span::styled(
                format!(" {first}  (ayrıntı: tonearm diag)"),
