...
//!
//! Snapshot'ları güncellemek için: `UPDATE_SNAPSHOTS=1 cargo test -p tonearm-cli`
...
fn temp_dir(label: &str) -> PathBuf {
    let unique = std::time::SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    let dir = std::env::temp_dir().join(format!("tonearm-cli-{label}-{unique}"));
    std::fs::create_dir_all(&dir).expect("geçici dizin");
    dir
}

/// `tonearm` ikilisini çalıştırır; `(stdout, stderr, başarılı_mı)`.
...
fn run_with_password(data_dir: &Path, password: &str, args: &[&str]) -> (String, String, bool) {
    let mut command = Command::new(env!("CARGO_BIN_EXE_tonearm"));
    command
        .arg("--data-dir")
        .arg(data_dir)
        .args(args)
        .env("TONEARM_PASSWORD", password)
        .env("TONEARM_MUSIC_DIRS", "/olmayan/dizin/tonearm-test");
    let output = command.output().expect("tonearm ikilisi çalışmalı");
    (
...
fn assert_snapshot(name: &str, stdout: &str) {
    ...
    assert_eq!(
        actual, expected,
        "\n{name} snapshot'ı değişti. Kasıtlıysa: UPDATE_SNAPSHOTS=1 cargo test -p tonearm-cli\n"
    );
}
...
///
/// Ağa çıkmıyor (`--no-verify`); taşıma katmanı `tonearm-core`'un
...
fn without_a_terminal_the_password_prompt_points_at_the_env_var() {
    ...
    // `run` TONEARM_PASSWORD ayarlamıyor ve test süreci bir tty'ye bağlı değil.
    let (_, stderr, ok) = run(
        &dir,
        &[
            "provider",
    ...
    assert!(
        stderr.contains("TONEARM_PASSWORD"),
...
fn secrets_are_listed_by_name_and_never_by_value() {
    ...
    let output = command
        ...
        .env("TONEARM_SECRET", "cok-gizli-deger")
        .env("TONEARM_MUSIC_DIRS", "/olmayan/dizin/tonearm-test")
        .output()
        .expect("tonearm ikilisi çalışmalı");
    assert!(
        output.status.success(),
