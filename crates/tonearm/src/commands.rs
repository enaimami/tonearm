...
use tonearm_core::playback::{PlaybackAnchor, QueueView, RepeatMode};
use tonearm_core::provider::remote::{self, NewServer, ServerKind};
...
///
/// Olay adı `tonearm://busy`, yükü işin adı (bitişte `null`). Zamanlayıcıyla
/// değil durum değişiminde gidiyor — D-033'ün kuralı burada da geçerli.
pub const BUSY_EVENT: &str = "tonearm://busy";

/// Uzun süren bir çekirdek çağrısını meşguliyet olayıyla sarar.
async fn busy<T, F>(app: &AppHandle, what: &str, task: F) -> tonearm_core::Result<T>
where
    F: Future<Output = tonearm_core::Result<T>>,
{
...
///
/// `Session::play` bloklayan yol (CLI'nin `tonearm play`'i); GUI onu değil
...
fn theme_error(text: String) -> CommandError {
    CommandError::new(tonearm_core::diag::Stage::ConfigLoad, &text)
}
...
// Komutlar CLI'nin `tonearm plugin ...` alt komutlarıyla birebir aynı çekirdek
...
pub async fn diag(
    state: State<'_, AppState>,
) -> CommandResult<Option<tonearm_core::diag::DiagReport>> {
    state
