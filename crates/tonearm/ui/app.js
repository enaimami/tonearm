// tonearm — masaüstü arayüzü (PLAN §3.2).
//
...
listen("tonearm://tick", async (event) => {
...
listen("tonearm://error", (event) => toast(event.payload));

listen("tonearm://busy", (event) => {
...
(async function boot() {
  ...
  if (env) {
    $("envBlock").textContent = [
      ...
      `müzik       : ${env.music_dirs.length > 0 ? env.music_dirs.join(", ") : "tanımlı değil (TONEARM_MUSIC_DIRS)"}`,
    ].join("\n");
  }
  renderQueue(await call("queue"));
  renderAnchor(await call("anchor"));
