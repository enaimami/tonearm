...
//!
//! ```bash
//! TONEARM_TORZNAB_URL=http://127.0.0.1:9696/1/api \
//! TONEARM_TORZNAB_KEY=... cargo test -p tonearm-plugin-torrent --test torznab_live
//! ```

#![allow(clippy::unwrap_used, clippy::expect_used, clippy::panic)]

use tonearm_plugin_torrent::torznab::Torznab;

fn client_or_skip(test: &str) -> Option<Torznab> {
    let Ok(url) = std::env::var("TONEARM_TORZNAB_URL") else {
        eprintln!(
            "{test}: atlandı — TONEARM_TORZNAB_URL yok. \
             Canlı sınama kullanıcının kendi Prowlarr/Jackett'ını ister."
        );
        return None;
    };
    let key = std::env::var("TONEARM_TORZNAB_KEY").unwrap_or_default();
    match Torznab::new(&url, &key) {
        ...
        Err(error) => {
            ...
            panic!("{test}: TONEARM_TORZNAB_URL geçersiz: {error}");
        }
    }
}
...
async fn a_real_search_returns_releases_that_carry_an_infohash() {
    ...
    let query = std::env::var("TONEARM_TORZNAB_QUERY").unwrap_or_else(|_| "radiohead".to_owned());
...
async fn a_rejected_key_is_an_error_not_an_empty_result() {
    let Ok(url) = std::env::var("TONEARM_TORZNAB_URL") else {
        eprintln!("reddedilen anahtar: atlandı — TONEARM_TORZNAB_URL yok");
        return;
    };
    if std::env::var("TONEARM_TORZNAB_KEY").is_err() {
    ...
    let Ok(client) = Torznab::new(&url, "kesinlikle-yanlis-bir-anahtar") else {
        panic!("TONEARM_TORZNAB_URL geçersiz");
    };
