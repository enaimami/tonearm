...
use tonearm_plugin_torrent::engine::Engine;
use tonearm_plugin_torrent::stream::StreamServer;
...
async fn seeded_engine(name: &str) -> (Arc<Engine>, String, Vec<u8>, PathBuf) {
    ...
    engine
        .remember(vec![(
            infohash.clone(),
            tonearm_plugin_torrent::engine::CatalogEntry {
...
async fn a_source_with_no_peers_gives_up_within_the_budget_and_says_why() {
    ...
    assert!(
        elapsed < tonearm_core::plugin::client::CALL_TIMEOUT,
