...
//! sırada bekler. Sessiz kalmasın diye uzun komutlar `tune://busy` olayı
//! gönderiyor (K9).
...
use tune_core::diag::Stage;
use tune_core::playback::{LiveSession, Player};
...
impl Core {
    ...
    /// dosyası bozuksa.
    pub fn open(config: Config) -> tune_core::Result<Self> {
        let session = Session::open(config)?;
        let registry = tune_core::provider::default_registry(session.config())?;
    ...
    pub fn refresh_registry(&mut self) -> tune_core::Result<()> {
        self.registry = tune_core::provider::default_registry(self.live.session().config())?;
        Ok(())
    }
}
...
/// hata zarfı. `tune_core::Error` seri hâle getirilemiyor (kaynak zinciri
...
impl From<tune_core::Error> for CommandError {
    fn from(err: tune_core::Error) -> Self {
        Self {
            stage: err.stage(),
