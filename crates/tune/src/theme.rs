...
//!
//! **Neden burada, `tune-core`'da değil.** Altın Kural'ın testi "bunu GUI de
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("tune-theme-test-{name}"));
    ...
    fn a_root_only_theme_is_not_marked_extended() {
        assert!(targets_only_root(":root { --tune-bg: #fff; }"));
    ...
    fn a_media_query_counts_as_extended() {
        ...
        assert!(!targets_only_root(
            "@media (prefers-color-scheme: light) { :root { --tune-bg: #fff; } }"
        ));
    }
    ...
    fn both_reference_themes_load_and_stay_inside_the_contract() {
        ...
        for entry in BUILTIN {
            ...
            assert!(css.contains("--tune-bg"), "{} token yazmıyor", entry.id);
        }
    }
    ...
    fn the_two_reference_themes_differ_on_more_than_color() {
        ...
        assert!(contrast.css.contains("--tune-radius-sm: 0"));
        assert!(contrast.css.contains("--tune-duration: 0ms"));
    }
    ...
    fn selecting_a_theme_survives_a_restart() {
        ...
        assert!(active.css.contains("--tune-bg"));
