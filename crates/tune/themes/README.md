...
Bir tema `tune`'un renklerini, köşe yarıçaplarını ve canlandırma süresini
...
`~/.local/share/tune`). Dizini oraya koyup **listeyi yenile** deyin —
...
|---|---|---|
| `--tune-color-scheme` | `dark` | `dark` \| `light` — onay kutusu, imleç, kaydırma çubuğu gibi motorun kendi çizdiği parçalar için |
| `--tune-bg` | `#12100e` | sayfa arka planı |
| `--tune-surface` | `#1b1815` | panel arka planı (üst çubuk, kenar çubuğu, oynatıcı) |
| `--tune-surface-raised` | `#221e1a` | etkileşimli/üzerine gelinen arka plan (girdi, aktif sekme, uyarı) |
...
| `--tune-success` | `#7bd88f` | çalıyor / başarı |
| `--tune-error` | `#ff6b6b` | hata |
| `--tune-info` | `#79c0ff` | arabelleğe alınıyor / bilgi |
| `--tune-radius-sm` | `6px` | kontrol köşe yarıçapı |
| `--tune-radius-lg` | `8px` | panel köşe yarıçapı |
| `--tune-duration` | `120ms` | canlandırma süresi (`0ms` = hareket yok) |
