...
> **Türkçe kopya.** Kanonik metin İngilizcedir: [`DECISIONS.md`](DECISIONS.md).
...
Eşikler `spike/tauri-gonogo/THRESHOLDS.tr.md`'de **ölçümden önce** yazıldı; sayıları
...
Ayrıntı ve tekrar üretme yordamı: `spike/tauri-gonogo/RESULTS.tr.md`.
...
- `docs/` bundan sonra **yayımlanan bir yüzey**. Oraya konan her dosya
  herkese açıktır; `writing-plugins.tr.md` de aynı kökten servis ediliyor.
...
- `docs/writing-plugins.tr.md` §9–10 (SoundCloud ve YouTube Music'in kullanım
...
## D-073 — Her şey İngilizce: kod, yazı ve belgeler; belgelerin Türkçe kopyaları

**Tarih:** 2026-09-25 · **Durum:** UYGULANDI (2026-09-25)

**Soru:** Organizasyonun profil README'si yazılırken kullanıcı: *"dili garanti
şekilde ingilizce yap. her repo için ingilizce olsun lütfen. hatta otur şimdi
türkçe yazılmış her şeyi ingilizce olarak yeniden yaz."* Yayım için: *"bitince
hepsini gönder ve ayrıca türkçe olanlarıda [dosya_ismi].tr.[uzantı_ismi] olarak
kaydet."*

**Karar:**

1. **Her şey İngilizce** — tanımlayıcılar (D-036'dan beri zaten) da yazı da:
   yorumlar, doc comment'ler, CLI yardımı ve çıktısı, arayüz, hata ve tanı
   mesajları (`ADIM:` ön eki artık **`STEP:`**), test adları ve test verisi,
   fixture'lar, iş akışları, paketleme, belgeler. D-036'nın yazı dili yarısının
   yerini alır. İki depo da: `headshell/headshell` ve `headshell/plugins`.
2. **Belgelerin Türkçe kopyası yanlarında durur**, `<ad>.tr.<uzantı>` olarak:
   README, CONTRIBUTING, CLAUDE, PLAN, DECISIONS, eklenti rehberi, alt dizin
   README'leri, tanıtım sayfası ve örnek kart. Her kopyanın başında bir not var:
   kanonik metin İngilizce; kopya 2026-09-25 tarihli hâlidir ve güncel
   tutulacağı garanti değildir. README ve tanıtım sayfası iki dili birbirine
   bağlar; Türkçe bir kopyanın içindeki belge bağlantıları öteki Türkçe
   kopyaları gösterir ki çapalar çalışsın.
3. **Kodun `.tr` kopyası yok.** Bir `tests/*.tr.rs`'i cargo test olarak
   derlerdi; kodun tek dili var. `.desktop` girdisi Türkçeyi freedesktop'un
   yoluyla (`Comment[tr]=`) taşır, ayrı bir dosyayla değil.
4. **Geçmiş commit mesajları olduğu gibi kalır.** Geçmişi yeniden yazmak her
   karmayı değiştirir ve sürüm etiketleri onları gösteriyor; yeni commit'ler
   İngilizce.
5. **Gerçek veri olduğu gibi kalır:** sanatçı ve parça adları (Şebnem Ferah,
   Müslüm Gürses, Ezhel'in Geceler'i), doğruluk kümesindeki bilerek Türkçe olan
   başlık, normalleştiricinin sınandığı Türkçe karakterler ve müzik dizini
   aramasının baktığı `Müzik` klasörü.

**Gerekçe:** D-036'nın çizdiği çizgi — "metni kullanıcı okur, o yüzden Türkçe"
— Türkçe bilen bir kitle varsayıyordu. Proje herkese açık (D-002), eklenti
kataloğu tanımadığımız yazarları davet ediyor (D-071) ve AUR paketleri zaten
İngilizceye ayrılmıştı (D-066'nın "bilinen ve kabul edilen tutarsızlığı").
Türkçe bilmeyen bir okur tek bir hata mesajını, eklenti rehberini ya da bu
defterdeki gerekçeleri okuyamıyordu.

Uygulamanın ayrıntısı — kodu yalnızca metni değiştirebilen araçlarla çevirmek,
çoğul ekleri, kartın İngilizcesi, yalnızca ASCII harfli Türkçeyi bulan sözlük
taraması, iki depolu yayım sırası (önce katalog: soundcloud 0.2.1, ytmusic
0.4.1), yeniden adlandırılan dosyalar ve kararsız ses testinin ölçümü — kanonik
metinde, `DECISIONS.md`'nin D-073'ünde.
